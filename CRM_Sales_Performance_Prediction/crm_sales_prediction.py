"""
CRM Sales Performance Prediction Model
This script builds a predictive model for sales performance based on CRM data.
"""
...
# Set paths
DATA_PATH = r'c:\Users\IversonScarlettTAC\Downloads\Predictive Analysis\Predictive Analytics for CRM Sales Performance'
OUTPUT_PATH = r'c:\Users\IversonScarlettTAC\Downloads\Predictive Analysis\Predictive_Analysis_Portfolio\CRM_Sales_Performance_Prediction'

def load_and_explore_data():
    """Load and explore the CRM sales dataset"""
    print("Loading CRM sales dataset...")
    ...
    # Display basic info
    print(f"Sales Pipeline shape: {sales_pipeline_df.shape}")
    print(f"Accounts shape: {accounts_df.shape}")
    print(f"Products shape: {products_df.shape}")
    print(f"Sales Teams shape: {sales_teams_df.shape}")
    
    print("\nFirst few rows of Sales Pipeline:")
    ...
    # Data types
    print("\nData types in Sales Pipeline:")
    print(sales_pipeline_df.dtypes)
    
    # Basic statistics
    print("\nBasic statistics for Sales Pipeline:")
    print(sales_pipeline_df.describe())
    
    # Merge datasets for a complete view
    # Merge sales pipeline with accounts
    if 'account_id' in sales_pipeline_df.columns and 'account_id' in accounts_df.columns:
        merged_df = pd.merge(sales_pipeline_df, accounts_df, on='account_id', how='left')
    else:
        merged_df = sales_pipeline_df.copy()
    
    # Merge with products if applicable
    if 'product_id' in merged_df.columns and 'product_id' in products_df.columns:
        merged_df = pd.merge(merged_df, products_df, on='product_id', how='left')
    
    # Merge with sales teams if applicable
    if 'team_id' in merged_df.columns and 'team_id' in sales_teams_df.columns:
        merged_df = pd.merge(merged_df, sales_teams_df, on='team_id', how='left')
    
    print("\nMerged dataset shape:")
    print(merged_df.shape)
    
    # Identify potential target variables (sales amount, win probability, etc.)
    potential_targets = []
    for col in merged_df.columns:
        if col.lower() in ['amount', 'sales_amount', 'deal_size', 'revenue', 'win_probability', 'expected_revenue']:
            potential_targets.append(col)
    
    if potential_targets:
        target_col = potential_targets[0]  # Use the first identified target
        print(f"\nUsing {target_col} as the target variable")
    else:
        # Try to identify a numerical column that might be the target
        num_cols = merged_df.select_dtypes(include=['float64', 'int64']).columns
        if len(num_cols) > 0:
            # Look for columns with 'amount' or 'value' in the name
            amount_cols = [col for col in num_cols if 'amount' in col.lower() or 'value' in col.lower()]
            if amount_cols:
    ...
    sns.histplot(merged_df[target_col].dropna(), kde=True)
    ...
    if 'stage' in merged_df.columns:
        ...
        sns.boxplot(x='stage', y=target_col, data=merged_df)
        plt.title(f'{target_col} by Sales Stage')
        plt.xticks(rotation=45)
        plt.tight_layout()
        plt.savefig(os.path.join(OUTPUT_PATH, 'target_by_stage.png'))
...
def preprocess_data(df, target_col):
    """Preprocess the CRM sales dataset for modeling"""
    print("\nPreprocessing data...")
...
def save_model_and_results(best_model, best_model_name, best_metrics, top_features):
    ...
    # Save the model
    joblib.dump(best_model, os.path.join(OUTPUT_PATH, 'crm_sales_model.pkl'))
    ...
    with open(os.path.join(OUTPUT_PATH, 'README.md'), 'w') as f:
        f.write(f"# CRM Sales Performance Prediction Model\n\n")
        f.write("## Overview\n")
        f.write("This project builds a machine learning model to predict sales performance metrics based on CRM data, including account information, sales pipeline stages, and team data.\n\n")
        f.write("## Dataset\n")
        f.write("The dataset contains CRM data including sales pipeline information, account details, product information, and sales team data.\n\n")
        ...
        if top_features:
            f.write("Top factors influencing sales performance:\n")
            for feature, importance in top_features[:5]:  # List top 5 features
                f.write(f"- {feature}: {importance:.4f}\n")
        f.write("\n## Files\n")
        f.write("- `crm_sales_prediction.py`: Main script for data analysis and model building\n")
        f.write("- `crm_sales_model.pkl`: Saved model file\n")
        f.write("- `model_metrics.txt`: Detailed model performance metrics\n")
        f.write("- `target_distribution.png`: Distribution of the target variable\n")
        f.write("- `target_by_stage.png`: Target variable by sales stage\n")
        f.write("- `correlation_heatmap.png`: Correlation between features\n")
        f.write("- `actual_vs_predicted.png`: Comparison of actual vs predicted values\n")
        f.write("- `feature_importance.png`: Visualization of feature importance\n")

def main():
    """Main function to run the CRM sales prediction analysis"""
    print("Starting CRM Sales Performance Prediction Analysis...")
    ...
    print("\nCRM Sales Performance Prediction Analysis completed successfully!")
